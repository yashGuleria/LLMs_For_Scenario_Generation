...
def validate_aircraft_count(root: ET.Element) -> Tuple[bool, int]:
    """Validate if scenario has Type 3 aircraft."""
    flightplans = root.findall(".//initial-flightplans")
    count = len(flightplans)
    is_valid = count >= 10
    if not is_valid:
        logger.error(f"Expected more than 9 aircraft, found {count}")
...
def main():
    ...
    scenarios_dir = "generated_scenarios_level2_1_temp0.2"
    ...
    report_filename = f"validation_report_level2_1_temp0.2_{timestamp}.txt"
